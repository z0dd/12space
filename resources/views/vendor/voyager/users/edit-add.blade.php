                ...
                <div class="col-md-8">
                    <div class="panel panel-bordered">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                <label for="gender_id">{{ __('voyager::generic.gender') }}</label>
                                <select class="form-control select2" id="gender_id" name="gender_id">
                                    @foreach (\App\Gender::all() as $gender)
                                        <option value="{{ $gender->id }}"
                                            {{ ($gender->id == $dataTypeContent->gender_id ? 'selected' : '') }}>{{ $gender->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="account_id">{{ __('voyager::generic.account') }}</label>
                                <select class="form-control select2" id="account_id" name="account_id">
                                    @foreach (\App\Account::all() as $account)
                                        <option value="{{ $account->id }}"
                                            {{ ($account->id == $dataTypeContent->account_id ? 'selected' : '') }}>{{ $account->name }}</option>
                                    @endforeach
                                </select>
                            </div>
